...
```{r}
# Replace NAs with invalid values
df.fcar <- df.mcar

na_rows <- which(is.na(df.mcar$jahre_im_unternehmen))

for (i in na_rows) {
  # If alter[i] is NA, skip to the next iteration
  if (is.na(df.fcar$alter[i])) {
    next
  }
  
  repeat {
    # Sample a value from df_faulty
...
```{r}
validation_fcar <- confront(df.fcar, rules)

valid_fcar <- values(validation_fcar)[, 1]

faulty_rows <- which(!valid_fcar)

all_faulty_rows <- union(na_rows, faulty_rows)
df_faulty_values <- df.fcar[all_faulty_rows, ]
```
