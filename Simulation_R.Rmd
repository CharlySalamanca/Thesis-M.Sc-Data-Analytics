---
title: An R Markdown document converted from "Simulation_R.ipynb"
...
```{r}
df <- read.csv("CAMPUS-File_VSE_2018_Arbeitnehmer.csv", sep = ";") 
```
...
```{r}
vis_miss(df.mcar, sort_miss = F, cluster = F) 
```
...
rules <- validator(
  jahre_im_unternehmen <= alter - 14
)

rules
```
# Option 1: Nicht-Verteilungstreue Simulation

```{r}
extreme_values <- c(seq(-100, -1, by = 1), seq(54, 100, by = 1))
non_numeric_placeholders <- rep(NaN, 100)
faulty_values <- c(extreme_values, non_numeric_placeholders)
df_faulty <- data.frame(jahre_im_unternehmen = faulty_values)



df_faulty$jahre_im_unternehmen
...
na_rows <- which(is.na(df.mcar$jahre_im_unternehmen))

for (i in na_rows) {
  df.fcar$jahre_im_unternehmen[i] <- sample(df_faulty$jahre_im_unternehmen, 1)
}
...
```{r}
df_FCAR <- list(
  idx_newNA = df_MCAR$idx_newNA,
  data.init = df_MCAR$data.init,
  data.incomp = df_MCAR$data.incomp,
  data.faulty = df.fcar  
)
...
validation_results <- confront(df.fcar, rules)
validation_summary <- summary(validation_results)
validation_summary
```
```{r}
### Rule funktioniert nicht, waurm auch immer.

#Alternative:
  sum(with(df.fcar, jahre_im_unternehmen > (alter - 14)), na.rm = TRUE)
```


# Option 2: Verteilungstreue Simulation

```{r}
set.seed(123)

valid_jahre <- df_1$jahre_im_unternehmen[!is.na(df_1$jahre_im_unternehmen)]
faulty_data <- sample(df_1$jahre_im_unternehmen[!is.na(df_1$jahre_im_unternehmen)], 200, replace = TRUE)
extreme_values <- sample((max(valid_jahre, na.rm = TRUE) + 1):(max(valid_jahre, na.rm = TRUE) + 20), 50, replace = TRUE)

faulty_data_combined <- c(faulty_data, extreme_values)
df_faulty <- data.frame(jahre_im_unternehmen = faulty_data_combined)

ggplot() +
  geom_density(aes(x = df_1$jahre_im_unternehmen, fill = "df"), alpha = 0.5, na.rm = TRUE) +
  geom_density(aes(x = df_faulty$jahre_im_unternehmen, fill = "df_faulty"), alpha = 0.5) +
  labs(
    title = "Valid and faulty data for jahre_im_unternehmen",
    x = "jahre_im_unternehmen",
    y = "Density"
  ) +
  scale_fill_manual(values = c("green", "red"), labels = c("Valid Data", "Faulty Data")) +
  theme_minimal()
...
for (i in na_rows) {
  # If alter is NA, skip 
  if (is.na(df.fcar$alter[i])) {
    next
  }
  
  repeat {
    # Sample from df_faulty
    sampled_value <- sample(df_faulty$jahre_im_unternehmen, 1)

    # Check if the sampled value violates the rule
    if (sampled_value > df.fcar$alter[i] - 14) {
      df.fcar$jahre_im_unternehmen[i] <- sampled_value
      break
    }
  }
}
...
```{r}
na_counts <- colSums(is.na(df.mcar))
na_counts
```
